...
export default function TwentyFiveYardDumpsterClient() {
  const { content: siteContent } = useSiteContent();
  const product = getDumpsterProducts(siteContent).find((p) => p.id === '25-yard-dumpster')!;
  const content = useDumpsterPageContent('25-yard-dumpster');

  const heroImg = siteContent.pageHeroes?.twentyFiveYard || content.image || {
    ...
    <>
      ...
      <PageHero
        breadcrumbs={[
          ...
          { label: '25 Yard Dumpster' },
        ]}
        headlineWhite="25 YARD ROLL-OFF"
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '36px', alignItems: 'flex-start' }}>
            ...
            <div>
              ...
              <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(1.9rem, 3.6vw, 2.5rem)', fontWeight: 800, color: '#0f172a', textTransform: 'uppercase', margin: '0 0 16px 0', lineHeight: 1.1 }}>
                BEST USES FOR A 25 YARD CONTAINER
              </h2>
      ...
      {/* 3. Quote Form */}
      <FreeQuoteForm id="quote" defaultService="25-yard-dumpster" />

      {/* 4. Closing CTA */}
      <ClosingCtaBanner
        headline="READY TO BOOK YOUR 25 YARD DUMPSTER?"
